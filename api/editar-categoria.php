...
// ----------------------------------------------------
$catId = 0;
if (isset($url[1]) && is_numeric($url[1])) $catId = (int)$url[1];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && postv('acao') === 'categoria_salvar') {
  ...
  mysqli_stmt_close($stmt);
      ...
      echo "<script> window.location.href='pediuservico/adm/categorias';</script>";
                  ...
                  <select class="form-select" name="grupo" required>
                    <?php
                      ...
                      while($g = mysqli_fetch_assoc($lista)) {
                        $sel = ((int)$g['codigo'] === (int)$rowCat['codgrupo']) ? "selected" : "";
...
const API_BASE = 'pediuservico/adm2/subcategoria_api.php';
...
if (quickSearchGroup && quickSearchResults) {
  quickSearchGroup.addEventListener('input', debounce(async () => {
    ...
    try {
      ...
      const response = await fetch(`pediuservico/adm2/api_get_categories.php?q=${encodeURIComponent(query)}`);
      const data = await response.json();
      
      if (data.ok && data.items && data.items.length > 0) {
        quickSearchResults.innerHTML = data.items.map(item => `
          <a href="pediuservico/adm2/editar-categoria/${item.codigo}" 
