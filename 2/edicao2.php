...
<head>
    ...
    <title>Minha Conta - USERVICE</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="tabs-container">
            <button class="tab " onclick="switchTab('dados')">
                👤 Dados Pessoais
            </button>
            <button class="tab" onclick="switchTab('categorias')">
                ⚙️ Categorias
            </button>
            <button class="tab active" onclick="switchTab('endereco')">
                📍 Endereço
            </button>
        </div>
        ...
        <div class="content-container">
            ...
            <div id="dados-content" style="display: none;">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="estado" id="estado" class="form-input" required>
                                    <?php
                                    $sqlUf = "SELECT * from estados ORDER BY Nome asc";
                                    $resUf = mysqli_query($con, $sqlUf);
                                    while ($row2 = mysqli_fetch_assoc($resUf)) { ?>
                                    <option value="<?php echo $row2['Uf']; ?>" <?php if($rowEdit['ESTADO']==$row2['Uf']) { echo "selected"; } ?>><?php echo $row2['Nome']; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="cidade" id="cidade" class="form-input" required>
                                    <?php
                                    $sqlCity = "SELECT * from cidades ORDER BY Nome asc";
                                    $sqlCity = mysqli_query($con, $sqlCity);
                                    while ($row = mysqli_fetch_assoc($sqlCity)) { ?>
                                    <option value="<?php echo $row['Nome']; ?>" <?php if($rowEdit['MUNICIPIO']==$row['Nome']) { echo "selected"; } ?>><?php echo $row['Nome']; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        ...
        // State change handler for cities
        document.getElementById('estado').addEventListener('change', function() {
            ...
            fetch('cidades.php', {
                ...
                body: 'id=' + encodeURIComponent(estadoId)
            })
