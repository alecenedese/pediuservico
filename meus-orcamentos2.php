...
<head>
    ...
    <title>Orçamentos Aceitos - USERVICE</title>
...
<style>
    ...
    border: 1px solid rgba(40,167,69,0.18);
            ...
            /* Message button with badge styling for accepted orders */
        .message-button {
            background: #6c757d;
            color: white;
            padding: 13px 16px;
        ...
        .message-badge[data-count="0"]::after {
            display: none;
        }
...
?>
    ...
    <!-- Menu sidebar identical to meus-orcamentos.php -->
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            <!-- Order cards with vertical mobile layout for accepted orders -->
            <?php
            ...
            SELECT g.titulo, p.codigo, s.titulo AS sub, p.local, p.tempo, p.descricao, p.lat, p.log, p.data_hora, cat.codcadastro, p.servicos, pg.nome, MIN(d.visto) as visto FROM 
            grupos g,
            ...
            AND d.aceito =  's'
            and d.codcadastro = pg.codcadastro
            AND d.codpedido = pg.codpedido
            AND p.codigo = pg.codpedido
            GROUP BY p.codigo
            ...
            if (mysqli_num_rows($queryEdit2) == 0) {
                echo '<div class="empty-state">
                    ...
                    <h3>Nenhum orçamento aceito</h3>
                    <p>Quando você aceitar pedidos, eles aparecerão aqui.</p>
                </div>';
            }
...
<div class="order-card status-aceito" data-codpedido="<?php echo $rowEdit1['codigo']; ?>" data-visto="<?php echo isset($rowEdit1['visto']) ? (int)$rowEdit1['visto'] : 0; ?>">
  ...
  <div class="order-info">
    <!-- TROQUE "PRESTADOR" por "CLIENTE" se no prestador fizer mais sentido -->
    <div class="info-item">
      <div class="info-icon icon-user">👤</div>
      <div class="info-content">
        <div class="info-label">CLIENTE</div>
        <div class="info-value"><?php echo $rowEdit1['nome'] ?? '---'; ?></div>
      </div>
    </div>
  ...
  <div class="description">
    ...
    <!-- Message button for accepted orders with unread message badge -->
                        <?php        
                        $listaG2 = mysqli_query($con, "select * from pega_contato where codpedido='" .$rowEdit1['codigo']. "' and codcadastro = '" .$rowEdit1['codcadastro']. "'"); 
                        $contaAceito = mysqli_fetch_array($listaG2);
                        $codClienteConversa = $contaAceito['codcliente'];
                        $contaPegacontato = mysqli_num_rows($listaG2);
                        $codcliente = $contaAceito['codcadastro'];
                        ?>

                        <a href="<?php if($contaPegacontato > 0) { ?>chat2.php?categoria=<?php echo $rowEdit1['sub']; ?>&subcategoria=<?php echo $rowEdit1['titulo']; ?>&user=<?php echo $nome ?>&user_id=<?php echo $codcliente; ?>&user_from=<?php echo $codClienteConversa; ?>&codpedido=<?php echo $rowEdit1['codigo']; } else { echo "meus-orcamentos2.php?alerta=1"; } ?>" class="message-button message-badge" data-count="<?php 
                        $listaGg = mysqli_query($con, "select count(opened) from chats 
                        WHERE
                        opened = 0
                        AND from_id = '".$codClienteConversa."'
                        AND to_id = '".$rowEdit['id']."'
                        AND codpedido = '".$rowEdit1['codigo']."'");
                        $rowgg = mysqli_fetch_array($listaGg); 
                        echo $rowgg['count(opened)'];	
                        ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
(function() {
    let badgeCount = <?php echo (int)$_badgeAceitos; ?>;

    function atualizarBadge() {
        const el = document.querySelector('a[href="meus-orcamentos2.php"] .tab-badge');
        if (badgeCount > 0) {
            if (el) { el.textContent = badgeCount; }
            else {
                const link = document.querySelector('a[href="meus-orcamentos2.php"]');
                if (link) {
